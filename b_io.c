...
b_io_fd b_open (char * filename, int flags)
	...
	//				 But make sure every file has its own buffer
        ...
        if( fd != -1){
                ...
                LBAread(fcbArray[fd].buf, 1, fcbArray[fd].fi->location);
        }
        fcbArray[fd].buf = malloc(512);
        fcbArray[fd].index = 0;
        fcbArray[fd].count = 0;
...
int b_read (b_io_fd fd, char * buffer, int count)
{
	...
	LBAread(fcbArray[fd].buf, 1, fcbArray[fd].fi->location);
        ...
        memcpy(buffer, fcbArray[fd].buf + fcbArray[fd].blockIndex, count);
        //printf("Printing my buffer -- %s\n", fcbArray[fd].buf);	
        fcbArray[fd].blockIndex += count; 
